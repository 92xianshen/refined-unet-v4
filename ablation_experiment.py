...
from model.crflayer_wobilateral import CRFLayer
...
output_path = '../ablation_study/result/'

def create_refined_unet_v4_wobilateral(
    input_channels, 
    num_classes, 
    theta_gamma=3.0, 
    spatial_compat=3.0, 
    ...
    # Create CRF layer
    crf = CRFLayer(num_classes, theta_gamma, spatial_compat, num_iterations)
...
model = create_refined_unet_v4_wobilateral(
...
def main():
    ...
    test_names = ['LC08_L1TP_113026_20160412_20170326_01_T1_train.tfrecords']
    print(test_names)
    ...
    with open(os.path.join(output_path, save_info_name), 'w') as fp:
        ...
        for test_name in test_names:
            ...
            # Save
            np.savez(os.path.join(output_path, save_npz_name), refinement)
            plt.imsave(os.path.join(output_path, save_png_name), refinement, cmap='gray')
